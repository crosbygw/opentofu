...
func TestMixin_Install(t *testing.T) {
	...
	expectedCommand := strings.Join([]string{
		"tofu init -backend=true -backend-config=key=my.tfstate -reconfigure",
		"tofu apply -auto-approve -input=false -var myvar=foo",
	}, "\n")
	...
	// fc, err := h.FileSystem.ReadFile(path.Join(wd, "terraform.tfvars.json")) <- todo
	require.NoError(t, err)
...
func TestMixin_InstallNoVarsBlock(t *testing.T) {
	...
	wd := h.Getwd()
	assert.Equal(t, wd, h.config.WorkingDir)
	//fc, err := h.FileSystem.ReadFile(path.Join(wd, "terraform.tfvars.json")) <- todo: fix
	require.NoError(t, err)
