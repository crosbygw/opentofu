...
func (m *Mixin) getOutput(ctx context.Context, outputName string) ([]byte, error) {
	// Using -json instead of -raw because opentofu only allows for string, bool,
	...
	cmd := m.NewCommand(ctx, "tofu", "output", "-json", outputName)
	cmd.Stderr = m.Err

	// Opentofu appears to auto-append a newline character when printing outputs
...
func (m *Mixin) commandPreRun(ctx context.Context, step *Step) error {
	...
	if m.DebugMode {
		fmt.Fprintln(m.Err, "Opentofu working directory is", m.Getwd())
	}
	...
	if err != nil {
		return fmt.Errorf("could not init opentofu, %s", err)
	}
	return nil
}
